    ...
    while let Some(node) = iter.next() {
        ...
        match *node {
            ...
            DerivedType::Function {
                ...
                for (p, param_declaration) in
                    parameter_type_list.param_declarations.iter().enumerate()
                {
                    let param_prefix = print_ast_prefix(
                        derived_type_prefix.clone(),
                        p == parameter_type_list.param_declarations.len() - 1,
                    );
                    println!("Parameter Declaration");

                    if let Some(declarator) = param_declaration.declarator {
                        print_ast_specifiers(
                            param_declaration.declaration_specifiers,
                            param_prefix.clone(),
                            false,
                        );
...
pub fn print_ast_list<'e, T: 'e, F: Fn(&T, String, bool)>(
    node_list: &[&T],
    prefix: String,
    is_last: bool,
    child_recurs: F,
...
pub fn print_ast_specifiers(specifiers_node: &Specifiers, prefix: String, is_last: bool) {
    ...
    match specifiers_node.type_specifier {
        ...
        TypeSpecifier::Struct {
            ...
            let print_struct_declaration =
                |declaration: &StructDeclarationNode, prefix: String, is_last: bool| {
                    print_ast_specifiers(declaration.specifiers, prefix.clone(), is_last);
                    print_ast_list(
                        declaration.struct_declarator_list,
                        prefix,
                        true,
                        |_, _, _| (),
                    );
            ...
            if let Some(declaration_list) = declaration_list {
                print_ast_list(
                    declaration_list,
                    specifiers_prefix.clone(),
                    is_last,
                    print_struct_declaration,
                );
            }
        }
            ...
            if let Some(enumerator_list) = enumerator_list {
                let print_enumerator = |enumerator: &Enumerator, prefix: String, is_last: bool| {
                    ...
                    if let Some(val) = enumerator.val {
                        println!(
                            "\x1b[1;90m{}\x1b[0m = \x1b[1;35m{}\x1b[0m",
                            enumerator.name, val
                        );
                    } else {
...
pub fn print_ast(start: &Node, prefix: String, is_last: bool) {
    ...
    match *start {
        ...
        Node::DeclarationList(list)
        | Node::StatementList(list)
        | Node::InitializerList(list)
        | Node::TranslationalUnit {
            external_declarations: list @ _,
        } => {
            match *start {
        ...
        Node::InitDeclaratorList(list) => {
            ...
            let print_init_declarator =
                |init_declarator: &InitDeclaratorNode, prefix: String, is_last: bool| {
